...
int query_max(ll segtree[],ll qlow,ll qhigh,ll low,ll high,ll pos)
{
	...
	ll mid=(low+high)/2;
	return(max(query_max(segtree,qlow,qhigh,low,mid,2*pos+1),query_max(segtree,qlow,qhigh,mid+1,high,2*pos+2)));
...
int main()
{
	...
	while(q--)
	{
		ll qlow,qhigh;
		cin>>qlow>>qhigh;
		cout<<query_max(segtree,qlow,qhigh,0,n-1,0)<<endl;
	}
