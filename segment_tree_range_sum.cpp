...
int query_sum(ll segtree[],ll qlow,ll qhigh,ll low,ll high,ll pos)
{
	...
	ll mid=(low+high)/2;
	return(query_sum(segtree,qlow,qhigh,low,mid,2*pos+1)+query_sum(segtree,qlow,qhigh,mid+1,high,2*pos+2));
...
int main()
{
	...
	while(q--)
	{
		ll qlow,qhigh;
		cin>>qlow>>qhigh;
		cout<<query_sum(segtree,qlow,qhigh,0,n-1,0)<<endl;
	}
